...
export default function App() {
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: Upload, 2: Analyzing, 3: Result
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (inputType === 'text') {
        ...
        // For PDF, we encode to base64. For text-based files, we read as text.
        if (file.type === 'application/pdf') {
          ...
          // Fallback for unsupported browser-side parsing (PPT, DOC, HWP)
          // Ideally, a backend handles this. For this demo, we guide user or try to interpret if possible,
          // but Gemini API mainly supports PDF/Images/Video/Audio/Text. 
          // We will throw a friendly error advising PDF conversion.
           throw new Error("브라우저 환경에서는 PDF, TXT, MD 파일만 직접 분석 가능합니다. PPT/DOC/HWP 등은 PDF로 변환하여 업로드하거나 내용을 텍스트로 붙여넣어 주세요.");
        }
      }

      // 2. Construct Prompt based on File 1 & File 2 Analysis
      const systemPrompt = `
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {step === 3 && result && (
          <ResultSection result={result} industry={selectedIndustry} onReset={handleReset} />
        )}
      </main>
    </div>
  );
...
function ResultSection({ result, industry, onReset }: { result: EvaluationResult, industry: string, onReset: () => void }) {
